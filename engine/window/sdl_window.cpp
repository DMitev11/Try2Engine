...
objects::Window sdl::createWindow(int height, int width, const char* title, std::vector<uint32_t> flags) {
    // Create window
    uint32_t flagsOr = 0;
    for each (uint32_t flag in flags) {
        flagsOr = flagsOr | flag;
    }
    SDL_Window* window = SDL_CreateWindow(title, width, height, flagsOr);
    if (window == NULL)
    {
